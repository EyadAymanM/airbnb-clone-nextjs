...
function Page({ params: { id } }) {
  ...
  const updateCategory = async () => {
    if (selectedCategory) {
      const listing = await updateListing(id, { category: selectedCategory })
      if (listing._id)
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 p-4 max-w-2xl mx-auto">
        {categories.map(({ icon, displayName ,_id }) => (
          <div
            ...
            className={`flex flex-col  justify-center p-4 border rounded-xl cursor-pointer transition-all overflow-hidden ${selectedCategory === _id
